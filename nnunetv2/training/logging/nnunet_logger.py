...
class MetaLogger(object):
    ...
    def finish(self):
        """Finish all attached loggers (e.g. flush and close W&B runs)."""
        self.local_logger.finish()
        for logger in self.loggers:
...
class LocalLogger:
    ...
    def log(self, key, value, epoch: int):
        ...
        # handle the ema_fg_dice special case! It is automatically logged when we add a new mean_fg_dice
        if key == 'mean_fg_dice':
            new_ema_pseudo_dice = self.get_value('ema_fg_dice', step=epoch-1) * 0.9 + 0.1 * value \
                if len(self.get_value('ema_fg_dice', step=None)) > 0 else value
